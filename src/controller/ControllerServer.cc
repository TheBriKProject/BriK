...
ControllerServer::ControllerServer(int max_chunks, int chunk_size,
    ...
    : _max_chunks(max_chunks), _chunk_size(chunk_size), _ts_min_rate(ts_min_rate),
      ...
      _chaff_frame(1, chunk_size), _disable_circ_change(disable_circ_change),
      _disable_dynamic_rate(disable_dynamic_rate)
{
...
void ControllerServer::handleSocksClientDataReady(FdPair *fdp)
{
    ...
    for (; chunk < total_chunks; chunk++) {
        ...
        nread = _sp->read_msg_client(fdp, din_ptr, din_sz);

        if (nread == 0 || (nread > 0 && nread < din_sz)) {
    ...
    /* Check for pending data messages, now that I've received one frame */
        ...
        bool have_recpt_frames = _client_manager.handleReceptionFrames([this]
            ...
            int status, nwrite;
            Frame *frame;
            char *dout_ptr; int dout_sz; int partial_offset;

            status = client->getReceivedFrame(frame);
            assert(status != RECP_NO_FRAME_AVAIL);

            partial_offset = client->getPartialWroteBytes(PARTIAL_LOCAL);
            ...
            if (status == RECP_DATA_FRAME) {
                ...
                if (client->getState() == CLIENT_STATE_ACTIVE) {
                    assert(partial_offset <= dout_sz);
                    nwrite = _sp->write_msg_local(fdp, dout_ptr + partial_offset, dout_sz);

                    if (nwrite == 0) {
                        return;
                    } else if (nwrite < 0) {
                        _sp->shutdown_connection(fdp);
                    } else if (nwrite > 0 && partial_offset + nwrite < dout_sz) {
                        client->setPartialWroteBytes(nwrite, PARTIAL_LOCAL);
                        return;
                    }

                    client->setPartialWroteBytes(0, PARTIAL_LOCAL); //Completed frame sent
...
void ControllerServer::handleCliRequest(std::string &request, std::string &response)
{
    ...
    } else if (cmd == "stats_clients_detail") {
        response = _client_manager.getClientDetails();
    }
    else if (cmd == "ts") {
...
void ControllerServer::handleTrafficShapingEvent()
{
    _client_manager.safeIterate([this](FdPair* fdp, Client* client) {
        ...
        unsigned int partial_offset = client->getPartialWroteBytes(PARTIAL_REMOTE);
        ...
        if ((ssl_partial_frame == -1 || ssl_partial_frame == FRAME_TYPE_CTRL) &&
            ...
            assert(status == FRAME_OK);
            assert(partial_offset <= chunk_sz);
            nwrite = _sp->write_msg_client(fdp, chunk_ptr + partial_offset, chunk_sz);

            if (nwrite == 0 || (nwrite > 0 && nwrite < chunk_sz)) {
                client->setWRTmpFrameType(FRAME_TYPE_CTRL);
                client->setPartialWroteBytes(nwrite, PARTIAL_REMOTE);
            } else {
                ...
                client->setPartialWroteBytes(0, PARTIAL_REMOTE);

                if (nwrite > 0 && nwrite == chunk_sz) {
                    #if STATS
