...
#define PARTIAL_LOCAL  (0)
#define PARTIAL_REMOTE (1)
...
class Client {
    ...
    public:
        Client() : _state(CLIENT_STATE_UNDEF), _k_min(CLIENT_K_MIN_UNDEF),
                   ...
                   _wr_tmp_frame_type(-1), _partial_wrote_bytes{0,0} {};
        ...
        unsigned int getPartialWroteBytes(int type) {
            return _partial_wrote_bytes[type];
        }

        void setPartialWroteBytes(unsigned int bytes, int type) {
            _partial_wrote_bytes[type] = bytes;
        }
    ...
    private:
        ...
        unsigned int _partial_wrote_bytes[2];
