    ...
    while (true) {
        ...
        if (!_direct_connect && FD_ISSET (sock_fd, &read_fd_set)) {
            ...
            if (_fds.empty()) {
                if (fd_bridge == -1) {
                    ...
                    status = fcntl(fd_bridge, F_SETFL, fcntl(fd_bridge, F_GETFL, 0) | O_NONBLOCK);
                    assert(status == 0);
                    ...
                    #if USE_SSL
                        switch (init_conn_handler(fd_bridge, fd_client, ssl)) {
                            case 1: _controller->handleSocksNewConnection(fd_pair); break;
                            case 0: /* Pending accept wait for select */ break;
                            case -1: shutdown_connection(fd_pair);
                        }
                    #else
                        if (init_conn_handler(fd_bridge, fd_client) == 1) {
                ...
                if (fd_bridge < 0) {
                    log("Couldn't re-establish Tor client segment to the bridge!");
                    exit(EXIT_FAILURE);
                }
        ...
        for (FdPair *fdp : _fds) {
            ...
            if (FD_ISSET (fd_bridge, &read_fd_set)) {
                #if USE_SSL
                    if (SSL_is_init_finished(fdp->getSSL()) != 1) {
                        switch (init_conn_handler(fd_bridge, fd_client, fdp->getSSL())) {
                            case 1: _controller->handleSocksNewConnection(fdp); break;
                            case 0: /* Pending accept wait for select */ break;
                            case -1: shutdown_connection(fdp);
                        }
                    }
                    else
                #endif
...
#if USE_SSL
    int SocksProxyClient::init_conn_handler(int fd_bridge, int fd_client, SSL* ssl)
#else
    int SocksProxyClient::init_conn_handler(int fd_bridge, int fd_client)
#endif
{
    int status, ssl_error;
    if (fd_bridge < 0) {
        ...
        return -1;
            ...
            if (status <= 0) {
                ssl_error = SSL_get_error(ssl, status);
                ...
                #if (LOG_VERBOSE & LOG_BIT_SSL)
                    log("SSL_connect error: %d", ssl_error);
                #endif
                return -1;
        ...
        // SSL connection fully established, can listen for Tor data
        if (fd_client != INV_FD)
...
#endif
{
    ...
    if (_direct_connect) {
        ...
        /* fd_client points to nowhere, since there are no real Tor proxy client.
            ...
            SSL *ssl = SSL_new(_ssl_ctx);
            SSL_set_fd(ssl, fd_bridge);
            FdPair *fd_pair = new FdPair(INV_FD, fd_bridge, ssl);
        #else
            FdPair *fd_pair = new FdPair(fd_client, fd_bridge);
        #endif
        ...
        #if USE_SSL
            if (init_conn_handler(fd_bridge, INV_FD, ssl) == 1) {
                _controller->handleSocksNewConnection(fd_pair);
            }
        #else
            if (init_conn_handler(fd_bridge, INV_FD) == 1) {
                _controller->handleSocksNewConnection(fd_pair);
            }
        #endif
    } else {
...
int SocksProxyClient::read_msg_client(FdPair *fd_pair, char *buff, int buffsize)
{
    ...
    int result = read(fd_pair->get_fd0(), buff, buffsize);
...
int SocksProxyClient::write_msg_client(FdPair *fd_pair, char *buff, int size)
{
    ...
    int result = write(fd_pair->get_fd0(), buff, size);
    if (result == -1 && (errno == EAGAIN || errno == EINTR)) {
        return 0;
    }
    return result;
...
int SocksProxyClient::read_msg_bridge(FdPair *fd_pair, char *buff, int buffsize)
{
    assert(buff != NULL && buffsize > 0);
    ...
    #if USE_SSL
        return fd_pair->tork_ssl_read(buff, buffsize);
    #else
        return readn(fd_pair->get_fd1(), buff, buffsize);
    #endif
}

int SocksProxyClient::write_msg_bridge(FdPair *fd_pair, char *buff, int size)
{
    assert(buff != NULL && size > 0);
    ...
    #if USE_SSL
        return fd_pair->tork_ssl_write(buff, size);
    #else
