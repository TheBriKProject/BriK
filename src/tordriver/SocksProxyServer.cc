...
void SocksProxyServer::log(const char *message, ...)
{
    ...
    fprintf(stdout, "LOG SEVERITY=notice MESSAGE=\"%s\"\n", vbuffer);
    fflush(stdout);
}

int SocksProxyServer::read_msg_client(FdPair *fd_pair, char *buff, int buffsize)
{
...
int SocksProxyServer::write_msg_client(FdPair *fd_pair, char *buff, int size)
{
...
int SocksProxyServer::read_msg_local(FdPair *fd_pair, char *buff, int buffsize)
{
    ...
    int result = read(fd_pair->get_fd1(), buff, buffsize);
...
int SocksProxyServer::restore_local_connection(FdPair *fd_pair) {
    ...
    int status = fcntl(fd_local, F_SETFL, fcntl(fd_local, F_GETFL, 0) | O_NONBLOCK);
    assert(status == 0);
...
#if USE_SSL
int SocksProxyServer::init_accept_handler(int fd_client, SSL* ssl) {
    ...
    if ((status = SSL_accept(ssl)) <= 0) {
        ...
        if (ssl_error == SSL_ERROR_WANT_READ || (ssl_error == SSL_ERROR_SYSCALL && errno == EAGAIN)) {
            return 0;
        }

        #if (LOG_VERBOSE & LOG_BIT_SSL)
            log("SSL_accept error: %d errno: %d", ssl_error, errno);
        #endif
        return -1;
    }
    ...
    while (true) {
        ...
        if (FD_ISSET (sock_fd, &read_fd_set)) {
            ...
            assert(status == 0);
                ...
                status = SSL_set_fd(ssl, fd_client);
                assert(status == 1);
                ...
                switch (init_accept_handler(fd_client, ssl)) {
        ...
        for (FdPair *fdp : _fds) {
            ...
            if (FD_ISSET (fd_client, &read_fd_set)) {
                    ...
                    if (SSL_is_init_finished(fdp->getSSL()) != 1) {
                        switch (init_accept_handler(fd_client, fdp->getSSL())) {
