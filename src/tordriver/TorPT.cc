...
void TorPT::log(LOG_SEVERITY severity, std::string message) {
    const char* severity_str[] = {"error", "warning", "notice", "info", "debug"};

    std::istringstream st(message);
    std::string line;
    while (std::getline(st, line, '\n')) {
        if (severity < WARNING) {
            std::cerr << str(boost::format("%s %s=ERROR %s=\"%s\"")
                % LOG % LOG_SEVERITY_STR % LOG_MESSAGE_STR % line) << std::endl;
        } else {
            std::cout << str(boost::format("%s %s=%s %s=\"%s\"")
                % LOG % LOG_SEVERITY_STR % severity_str[severity] %
                LOG_MESSAGE_STR % line) << std::endl;
        }
    }
}
