...
void *TorController::threadEvent()
{
    ...
    while (true) {
        ...
        if (parsed_event.size() > 3 && parsed_event[1] == "STREAM" &&
            parsed_event[3] == "CONTROLLER_WAIT") {

            event._type = TCTL_EVENT_STREAM_CONTROLLER_WAIT;
            event._stream = std::stoi(parsed_event[2]);
        }
        if (parsed_event.size() > 3 && parsed_event[1] == "STREAM" &&
            parsed_event[3] == "DETACHED") {

            event._type = TCTL_EVENT_STREAM_DETACHED;
            event._stream = std::stoi(parsed_event[2]);
            if (parsed_event[6] == "REASON=TIMEOUT") {
                event._reason = TCTL_STREAM_REASON_TIMEOUT;
            } else if (parsed_event[6] == "REASON=END") {
                event._reason = TCTL_STREAM_REASON_END;
            } else {
                event._reason = TCTL_STREAM_REASON_OTHER;
            }
        }
        if (parsed_event.size() > 3 && parsed_event[1] == "STREAM" &&
            parsed_event[3] == "CLOSED") {
